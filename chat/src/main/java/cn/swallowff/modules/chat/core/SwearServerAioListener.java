 ...
 * 用户根据情况来完成该类的实现
 */
public class SwearServerAioListener extends WsServerAioListener {
	private static Logger log = LoggerFactory.getLogger(SwearServerAioListener.class);

	public static final SwearServerAioListener me = new SwearServerAioListener();

	private SwearServerAioListener() {
...
//			log.info("onBeforeClose\r\n{}", channelContext);
		...
		if (wsSessionContext != null && wsSessionContext.isHandshaked()) {
			...
			//群发
			Tio.sendToGroup(channelContext.groupContext, ServerConfig.GROUP_ID, wsResponse);
		}
	}
