layui.config({
    ...
    table.render({
        ...
        url: setter.ctxPath + '/sys/user/list.ajax',  //请求路径
    ...
    var $ = layui.$, active = {
        batchdel: function () {
            ...
            layer.confirm('确定删除吗？', function (index) {
                //执行 Ajax 后重载
                admin.req({
                    url: setter.ctxPath + '/sys/user/batchDel',
                    method: 'POST',
        ...
        add: function () {
            layer.open({
                type: 2
                , title: '添加用户'
                , content: setter.ctxPath + '/sys/user/add.html'
                , maxmin: true
                , area: ['900px', '600px']
                , btn: ['确定', '取消']
    ...
    window.deleteRow = function (id) {
        $.ajax({
            url: setter.ctxPath + '/sys/user/delete',
            method: 'POST',
    ...
    window.editRow = function (id) {
        layer.open({
            type: 2
            , title: '編輯用户'
            , content: setter.ctxPath + '/sys/user/edit.html?id=' + id
            , maxmin: true
            , area: ['800px', '600px']
            , btn: ['确定', '取消']
            , yes: function (index, layero) {
    ...
    window.setupRole = function (id) {
        layer.open({
            ...
            yes: function (index) {
                ...
                $.ajax({
                    url: setter.ctxPath + '/sys/user/batchSetupRoles',
                    method: 'post',
                    traditional: true,
                    data: {
                        userId: id,
    ...
    var renderDtree = function (userId) {
        dtree.render({
            ...
            // icon: ["-1","5"],   //二级图标
            // ficon: ["-1","5"],   //一级图标
            url: setter.ctxPath + "/sys/role/userDtree.ajax?userId=" + userId, // 使用url加载（可与data加载同时存在）
            // height: '150px',
            method: 'GET',
