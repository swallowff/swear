layui.config({
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.code == setter.response.statusCode.ok) {
                ...
                form.render('select', 'LAYF-csGroupTeamUser-form-add'); //刷新select选择框
            }
        }
    });
    ...
    //监听提交
    form.on('submit(LAYF-csGroupTeamUser-form-add-submit)', function (data) {
        var field = data.field; //获取提交的字段
        var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引

        //提交 Ajax 成功后，关闭当前弹层并重载表格
        $.ajax({
            url: setter.ctxPath + '/cs/csGroupTeamUser/add.ajax',
            data: field,
            method: 'POST',
            success: function (res) {
                if (res.code == setter.response.statusCode.ok) {
