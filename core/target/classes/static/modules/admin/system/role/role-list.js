layui.config({
    ...
    table.render({
        ...
        url: setter.ctxPath + '/sys/role/list.ajax',  //请求路径
    ...
    var $ = layui.$, active = {
        batchdel: function () {
            ...
            layer.confirm('确定删除吗？', function (index) {
                ...
                //执行 Ajax 后重载
                admin.req({
                    url: setter.ctxPath + '/sys/role/batchDel',
                    method: 'POST',
        ...
        add: function () {
            layer.open({
                type: 2
                , title: '添加角色'
                , content: setter.ctxPath + '/sys/role/add.html'
                , maxmin: true
                , area: ['550px', '550px']
                , btn: ['确定', '取消']
    ...
    window.deleteRow = function (id) {
        $.ajax({
            url: setter.ctxPath + '/sys/role/delete',
            method: 'POST',
    ...
    window.editRow = function (id) {
        layer.open({
            type: 2
            , title: '編輯角色'
            , content: setter.ctxPath + '/sys/role/edit.html?id=' + id
            , maxmin: true
            , area: ['550px', '550px']
            , btn: ['确定', '取消']
            , yes: function (index, layero) {
    ...
    window.setupAuth = function (id) {
        layer.open({
            ...
            yes: function (index) {
                ...
                $.ajax({
                    url: setter.ctxPath + '/sys/role/batchSetupAuth',
                    method: 'post',
                    traditional: true,
                    data: {
                        roleId: id,
    ...
    var renderRoleDtree = function (roleId) {
        dtree.render({
            ...
            // icon: ["1","5"],   //二级图标
            // ficon: ["1","5"],   //一级图标
            url: setter.ctxPath + "/sys/menu/roleDtree.ajax?roleId=" + roleId, // 使用url加载（可与data加载同时存在）
            // height: '150px',
            method: 'GET',
