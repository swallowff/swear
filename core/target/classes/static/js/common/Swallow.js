var Swallow = {
    ...
    info: function (info) {
        Swallow.alert(info, 0);
    },
    success: function (info) {
        Swallow.alert(info, 1);
    },
    error: function (info) {
        Swallow.alert(info, 2);
    },
    ...
    baseAjax: function (url, tip) {
        var ajax = new $ax(Swallow.ctxPath + url, function (data) {
            Swallow.success(tip + "成功!");
        }, function (data) {
            Swallow.error(tip + "失败!" + data.responseJSON.message + "!");
        });
        return ajax;
    },
    changeAjax: function (url) {
        return Swallow.baseAjax(url, "修改");
    },
        ...
        $.ajaxSetup({
            ...
            complete: function (XMLHttpRequest, textStatus) {
                ...
                if (sessionstatus == "timeout") {
                    //如果超时就处理 ，指定要跳转的页面
                    window.location = Swallow.ctxPath + "/global/sessionError";
                }
            }
        });
    },
