...
(function() {
	...
	var $WebUpload = function(pictureId) {
		...
		this.uploadUrl = Swallow.ctxPath + '/mgr/upload';
		...
		bindEvent : function(bindedObj) {
			...
			bindedObj.on('error', function(type) {
				if ("Q_EXCEED_SIZE_LIMIT" == type) {
					Swallow.error("文件大小超出了限制");
				} else if ("Q_TYPE_DENIED" == type) {
					Swallow.error("文件类型不满足");
				} else if ("Q_EXCEED_NUM_LIMIT" == type) {
					Swallow.error("上传数量超过限制");
				} else if ("F_DUPLICATE" == type) {
					Swallow.error("图片选择重复");
				} else {
					Swallow.error("上传过程中出错");
				}
			});
