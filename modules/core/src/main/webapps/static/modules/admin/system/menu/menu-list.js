layui.config({
    ...
    table.render({
        ...
        url: setter.ctxPath + '/sys/menu/list.ajax',  //请求路径
    ...
    var $ = layui.$, active = {
        batchdel: function () {
            ...
            layer.confirm('确定删除吗？', function (index) {
                //执行 Ajax 后重载
                admin.req({
                    url: setter.ctxPath + '/sys/menu/batchDel',
                    method: 'POST',
        ...
        add: function () {
            layer.open({
                type: 2
                , title: '添加菜单'
                , content: setter.ctxPath + '/sys/menu/add.html'
                , maxmin: true
                , area: ['550px', '550px']
                , btn: ['确定', '取消']
    ...
    window.deleteRow = function (id) {
        $.ajax({
            url: setter.ctxPath + '/sys/menu/delete',
            method: 'POST',
    ...
    window.editRow = function (id) {
        layer.open({
            type: 2
            , title: '編輯菜单'
            , content: setter.ctxPath + '/sys/menu/edit.html?id=' + id
            , maxmin: true
            , area: ['550px', '550px']
            , btn: ['确定', '取消']
            , yes: function (index, layero) {
